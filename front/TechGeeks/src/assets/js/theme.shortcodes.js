...
function cloe_brooks_sc_init(container) {
	...
	if (window.cloe_brooks_theme_sc_init) cloe_brooks_theme_sc_init(container);
...
/*
	...
	if (navi.length > 0) {
		navi.on('click', '.sc_slider_prev,.sc_scroll_prev', function(e) {
			...
			CLOE_BROOKS_STORAGE['swipers'][id].slidePrev();
			e.preventDefault();
			return false;
		})
        .on('click', '.sc_slider_next,.sc_scroll_next', function(e){
			...
			if (swiper.length == 0) swiper = jQuery(this).parents('.sc_slider_controls_wrap,.sc_scroll_controls_wrap').siblings('.swiper-slider-container');
			var id = swiper.attr('id');
			CLOE_BROOKS_STORAGE['swipers'][id].slideNext();
			e.preventDefault();
			return false;
		});
	}
...
function cloe_brooks_sc_form_validate(form){
	...
	if (!error && url!='#') {
		jQuery.post(url, {
			...
			if( form.find(".result").length == 0 ) form.append('<div class="result sc_infobox_style_success sc_infobox sc_infobox_style_success"></div>');
