...
if(beforeStep){this.removeEventListener('click',open);classie.removeClass(self.el,'photostack-start');setTransition()}
else{self.openDefault=!0;setTimeout(setTransition,25)}
self.started=!0;self._showPhoto(self.current)};if(beforeStep){this._shuffle();this.el.addEventListener('click',open)}
else{open()}
...
else{callback()}}})});window.addEventListener('resize',function(){self._resizeHandler()})}
...
self.isRotating=!1;if(typeof callback==='function'){callback()}};if(this.flipped){classie.removeClass(this.navDots[this.current],'flip');if(support.preserve3d){this.currentItem.style.WebkitTransform='translate('+this.centerItem.x+'px,'+this.centerItem.y+'px) rotateY(0deg)';this.currentItem.style.transform='translate('+this.centerItem.x+'px,'+this.centerItem.y+'px) rotateY(0deg)'}
else{classie.removeClass(this.currentItem,'photostack-showback')}}
else{classie.addClass(this.navDots[this.current],'flip');if(support.preserve3d){this.currentItem.style.WebkitTransform='translate('+this.centerItem.x+'px,'+this.centerItem.y+'px) translate('+this.sizes.item.width+'px) rotateY(-179.9deg)';this.currentItem.style.transform='translate('+this.centerItem.x+'px,'+this.centerItem.y+'px) translate('+this.sizes.item.width+'px) rotateY(-179.9deg)'}
else{classie.addClass(this.currentItem,'photostack-showback')}}
