jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(CLOE_BROOKS_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == CLOE_BROOKS_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(CLOE_BROOKS_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				cloe_brooks_message_success('', CLOE_BROOKS_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(CLOE_BROOKS_STORAGE['strings']['editor_caption_close']);
			} else {
				cloe_brooks_message_warning(rez.error, CLOE_BROOKS_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		cloe_brooks_message_confirm(CLOE_BROOKS_STORAGE['strings']['editor_delete_post'], CLOE_BROOKS_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(CLOE_BROOKS_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					cloe_brooks_message_success('', CLOE_BROOKS_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = CLOE_BROOKS_STORAGE['site_url'];
						}, 1000);
				} else {
					cloe_brooks_message_warning(rez.error, CLOE_BROOKS_STORAGE['strings']['editor_delete_error']);
				}
			});
