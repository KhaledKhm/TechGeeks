...
function cloe_brooks_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		cloe_brooks_custom_options_show_loader();
		//location.reload();
		var loc = jQuery('#co_site_url').val();
		var params = CLOE_BROOKS_STORAGE['co_add_params']!=undefined ? CLOE_BROOKS_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#cloe_brooks-customizer-styles-'+option).length > 0 ? jQuery('#cloe_brooks-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="cloe_brooks-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
